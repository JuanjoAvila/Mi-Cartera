...
1. **Re-probar en el móvil** la beta 4.12.0 (APK 35) con el panel de revisión. Checklist clave: scroll en Deudas/Metas → deslizar; banco caído → banner en Cartera y rojo en Mis bancos; TR desconectado; cerrar perfil en caliente; Ajustes muestra web + app.
2. **Si aprueba: promocionar a producción** (`promote-beta.yml`) y publicar la APK 35 como release de producción.
3. Si rechaza, el informe del panel dice qué falla; otra beta (el progreso se resetea por compilación; los ✓ idénticos se heredan).
