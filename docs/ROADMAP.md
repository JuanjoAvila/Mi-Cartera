...
| **Abrir el perfil: los 175 ms que quedan** | Ya no es JS (el perfil de CPU deja la app por debajo del 1 %): es trabajo del navegador al hacer visible una pantalla de ~1.680 px. Si hiciera falta bajarlo más, ahí es donde hay que mirar — y NO en el velo, la sombra, la animación del avatar ni el radio, que están medidos y descartados uno por uno (números en el `CHANGELOG`). |
