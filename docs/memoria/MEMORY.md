...
- [Roadmap](mi-cartera-roadmap.md) — **v4.13.0 en `beta` con CUATRO tandas pendientes de su veredicto** (`import`, `gestos`, `arranque`, `bancos`); producción = **4.12.1** + APK 35. Tirón al deslizar y stopper del perfil **CERRADOS por él**. ⚠ Las tandas de la 4.13.0 van mezcladas en `beta`: esa ronda solo sube ENTERA — trocear exige rama `tanda/<id>` **antes** del primer commit (`docs/TESTING.md`). MyInvestor nativo y validar la entrada de las Edge Functions, pendientes. Era v4 (3.101→4.0.10) la hizo CURSOR.
