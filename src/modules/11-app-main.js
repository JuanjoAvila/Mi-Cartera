function App(){
  ...
  const enterScrollHost=function(i){
    ...
    el.style.willChange="";   // suelta la capa que pidió `onStart` (el porqué, allí)
    const pgs=el.children;
    for(let k=0;k<pgs.length;k++){
  ...
  const onStart=(e)=>{
    ...
    ensureScrollHost();
    /* LA CAPA DEL CARRUSEL SE PIDE AQUÍ, CON EL DEDO TODAVÍA QUIETO (2026-08-18).
       Medido en su OnePlus 13 a 120 fps (12 gestos de 12): en el PRIMER fotograma del gesto de
       pestañas el destello se apaga 1–3 frames —−16 niveles de luminancia en la esquina del glow,
       −8 en el fondo— y vuelve solo. Oscurece el DOBLE donde vive el destello que donde no, así
       que no es la pantalla bajando de brillo: es algo que daba luz y desaparece un frame.
       Y el movimiento en pantalla en el frame anterior es 0,00: la pantalla está QUIETA, o sea
       que no es el desliz ni el compositor arrastrando páginas. Es el cambio de modo de
       `leaveScrollHost()`, que en un solo bloque síncrono destruye la capa `position:fixed` del
       host y crea la del track (`will-change:auto` → `transform`).
       Pedirla antes no mueve un píxel: `will-change` no cambia layout ni pintado, solo avisa al
       compositor. Es el mismo criterio ya medido para los segmentos de Plan en `shell.html`
       («el coste existe; lo único que se puede elegir es CUÁNDO se paga») — y con el dedo quieto
       no se ve. Se suelta en `enterScrollHost`: una capa permanente cuesta memoria de vídeo.
       ⚠ Si esto no baja los picos de −16, la hipótesis del compositor es falsa: NO acumular otro
       parche encima, medir de nuevo. Método y criterio de verde en `docs/briefs/brief-claude-destello.md`. */
    try{ if(trackRef.current) trackRef.current.style.willChange="transform"; }catch(e){}
