...
/** Devuelve {filas, tipo}. Acepta .xlsx y .csv/.tsv/.txt. Los .xls VIEJOS (formato binario de
 *  Office 97) no: son otro formato completamente distinto, y decirlo claro vale más que fallar
 *  con «no he podido leerlo» — el arreglo es «Guardar como → .xlsx», que se hace en dos toques. */
function hojaLeer(file){
  const nom=(file&&file.name||"").toLowerCase();
  if(/\.xls$/.test(nom)) return Promise.reject(new Error("xls-viejo"));
...
function SheetImport({state, set, onClose, showToast, goGastos}){
  ...
  const cargar=function(file){
    ...
    hojaLeer(file).then(function(r){
      ...
      setErr(m==="xls-viejo"?t("ih_err_xls"):m==="no-zip"?t("ih_err_zip"):m==="vacio"?t("ih_err_vacio"):t("ih_err_leer"));
